#!/usr/bin/env bash
# Linux cross-platform test suite for platform-iac-modules.
#
# Canonical CI entry point:
#   task test:cross-platform:linux
#
# The suite validates the Linux toolchain and then runs the same
# catalog-driven smoke, static-analysis, and environment gates used by the
# repository's local and GitHub Actions workflows.
...
usage() {
  ...
  printf '%s\n' \
    ...
    "" \
    "Run the Linux cross-platform gates for platform-iac-modules." \
    ...
    "receives a transcript at test-linux.log." \
    "" \
    "Exit codes:" \
    "  0  all gates passed" \
...
version_pin() {
  local tool=$1
  awk -v tool="$tool" '$1 == tool { print $2; exit }' \
    "$TOOL_VERSIONS_FILE"
}
...
check_version() {
  ...
  if [[ "$actual" == "$expected" ]]; then
    ...
    error "${label} version mismatch: found ${actual}, expected ${expected}"
...
main() {
  ...
  if [[ "$(uname -s)" != "Linux" ]]; then
    error "This suite must run on Linux; detected $(uname -s)"
    exit 2
  fi
  ...
  mkdir -p -- "$OUTPUT_DIR"
  LOG_FILE="${OUTPUT_DIR}/test-linux.log"
  touch "$LOG_FILE"
  exec > >(tee -a "$LOG_FILE") 2>&1

  info "Repository: ${REPO_ROOT} ($(uname -m))"
